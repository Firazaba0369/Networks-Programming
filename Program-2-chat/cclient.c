...
void processNumHandles(uint8_t *packet);
void processHandles(uint8_t *packet);
void processDestHandleError(uint8_t *packet);
...
int main(int argc, char *argv[]) {
    ...
    checkArgs(argc, argv);
    strcpy(handle.handle_name, argv[1]);
    if (strlen(handle.handle_name) > MAX_HANDLE - 1) {
        fprintf(stderr,
...
void processMsgFromServer(int socketNum) {
    ...
    // process the packet
    int status = processPacket(packet);
    if (status < 0) {
        return;
    }
}
...
int processPacket(uint8_t *packet) {
    ...
    case NUM_HANDLES_FLAG:
        processNumHandles(packet);
        return 1;
    case LIST_HANDLES_FLAG:
        processHandles(packet);
        return 1;
    case LIST_COMPLETE_FLAG:
        return 1; // no action needed
    case DEST_HANDLE_ERROR:
        processDestHandleError(packet);
        return -1;
    default:
...
void processDestHandleError(uint8_t *packet) {
    ...
    dest_handle_name[dest_handle_len] = '\0';

    // print the destination handle error
    printf("Destination Handle '%s' does not exist\n", dest_handle_name);
...
int processMulticastCommand(handle_t *handle, uint8_t *buffer,
    ...
    num_handles = atoi(token); // number of destination handles
