...
int main(){
    ...
    for(int i = 1; i < 12; i++){
        // Add a new handle
        sprintf(new_handle.handleName, "test_handle%d", i);
    ...
    sprintf(fake_handle.handleName, "fake_handle");
    fake_handle.socket_num = 14;

    // Lookup the handle
    int index = lookupHandle(&handle_table, new_handle);
    if (index != -1) {
        printf("Handle found at index: %d\n", index);
        printf("Handle Name: %s, Socket Number: %d\n",
               handle_table.handles[index].handleName,
